...
def pde(x, y):
    y1 = 0.25 + y*(0.75-0.25)
    fac = 99.996996996997/(600*600)#Factor to be included to convert the Diffusivity into Dimensionless Form
...
def ZeroBC(x):
    return np.zeros((len(x),1))#Homogeneous Neumann Boundary Condition
...
def initial_function(x):
	...
	for i in range(1,len(a),4):
        	res += a[i]*np.sin(a[i+1]*x[:,0:1] )+a[i+2]*np.cos(a[i+3]*x[:,0:1] )#Intial Profile after 15hrs fitted with fourier series
	return res
        
def midpoint_constraint(inputs,outputs,X):
        y = 0.25 + outputs*(0.75-0.25)
        return 12.012012012012011*torch.exp(y+0.5*y*y+0.25*y*y*y)-d0*torch.exp(a*y + b*y*y + c*y*y*y)#Value od interdiffusion is known at the centre of the domain at all times which is fed a constraint
...
bc = dde.icbc.NeumannBC(geomtime, ZeroBC, lambda _, on_boundary: on_boundary)#Here we have Homogeneous Neumann BCs
...
constraint = dde.OperatorBC(geom,midpoint_constraint,lambda x,_: dde.utils.isclose(x[0],0.5))#Constraint is applied at the midpoint
...
#checkpointer = dde.callbacks.ModelCheckpoint("DiffusionBestModel.ckpt", verbose=1, save_better_only=True,period=10000)
losshistory, train_state = model.train(iterations=1000000,callbacks=[variable],model_save_path="NormalizedDiffusionModelLateStart")
